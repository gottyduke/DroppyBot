    ...
    def generate_help_info(self):
        help_info = [
            discord.Embed(
                ...
                title="GPT",
                description="""
            使用chatGPT进行文字补全, powered by OpenAI
...
{self.compiled_gpt4_cmd} "命里有时终须有，命里无时莫强求"这句话用俄语如何翻译? 要求尽量信达雅, 并逐句中俄双语对照和解释
        ...
        help_info.append(
            discord.Embed(
                ...
                title="如何选择不同的模型?",
                description="""
            本机器人🔧接入了OpenAI API, 自3月27日也获得了`gpt-4`模型API的内测资格, 因此提供`gpt-3.5-turbo`和`gpt-4`两种模型
            
            其中`gpt-3.5-turbo`是OpenAI默认chatGPT模型, 而`gpt-4`则是其之后的高级付费模型, 语言能力和逻辑思维得到了很大的改善
            
            两gpt模型的使用没有网页端的每小时条数限制. **但是**, 作为一个计费制的API, 在整活类问题/逻辑需求不强的使用场景下请调用`gpt-3.5-turbo`模型, 正常使用时则调用`gpt-4`模型
            """,
            )
        )
        help_info.append(
...
{self.compiled_gptinit_cmd} 你将扮演一个猫娘, 称呼我为主人, 开头和结尾都要使用喵语气和可爱风格
    ...
    def __init__(self):
        ...
        self.user_token = next(
            model
            for model in self.config.gpt.model.spec
            if model["name"] == self.config.gpt.model.default
        )["max_token"]
        self.user_token = int(
            self.user_token * self.config.gpt.contextual.max_ctx_percentage
        )
